...
public class VtnInsertaLibro extends JPanel {
	...
	public VtnInsertaLibro() {

		setLayout(null);

		JLabel lblLibroNuevo = new JLabel("LIBRO NUEVO");
		lblLibroNuevo.setForeground(new Color(44, 62, 80));
		lblLibroNuevo.setFont(new Font("Segoe UI", Font.BOLD, 22));
		lblLibroNuevo.setBounds(10, 21, 199, 33);
		add(lblLibroNuevo);

		JLabel lblNewLabelCantPaginas = new JLabel("Nº paginas:");
		lblNewLabelCantPaginas.setForeground(new Color(44, 62, 80));
		lblNewLabelCantPaginas.setFont(new Font("Segoe UI", Font.BOLD, 15));
		lblNewLabelCantPaginas.setBounds(10, 299, 94, 20);
		add(lblNewLabelCantPaginas);
		...
		scrollPane.setViewportView(textAreaDescrip);
		...
		btnInsertar.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		        ...
		        try {
		            ...
		            miLibro.setTitulo(textFieldTitulo.getText());
		            miLibro.setGenero(Multimedia.GENERO.valueOf(cmbGenero.getSelectedItem().toString()));

		            if (anio < 1946 || anio > 2026) {
		                JOptionPane.showMessageDialog(null, "Año de publicación del libro incorrecto!!");
		            ...
		            miLibro.setAutor(textFieldAutor.getText());
		            miLibro.setPuntuacion(sliderPuntuacion.getValue());
		            miLibro.setDescripcion(textAreaDescrip.getText());

		            String fechaInicioStr = txtFechaInicio.getText();
		            String fechaFinStr = textFieldFechaFin.getText();
		            int cantPaginas = Integer.parseInt(textFieldCantPaginas.getText());
		            ...
		            miLibro.setFechaInicio(fechaInicioStr);
		            miLibro.setFechaFin(fechaFinStr);
		            miLibro.setPersonajes(txtPersonajes.getText());
		            miLibro.setNumPaginas(cantPaginas);

		            int valor = JOptionPane.showConfirmDialog(null, "¿Desea guardar el libro?");
		            if (valor == JOptionPane.OK_OPTION) {
		                correcto = miBd.insertaDatosLibro(miLibro);
		                if (correcto) {
		                    JOptionPane.showMessageDialog(null, "Libro guardado correctamente");
		                    limpiarCampos();
		                } else {
		                    JOptionPane.showMessageDialog(null, "No se ha guardado el libro");
		                }
		            } else {
		...
		textFieldFechaFin.setBounds(163, 504, 111, 20);
		...
		textFieldCantPaginas.setBounds(114, 302, 86, 20);
		add(textFieldCantPaginas);
